...
namespace JZT.Utility.Service
{
	...
	public class CustomXmlSerializer
	{
		...
		{
            if (source is DataRow)//Îº¸Õ20100812
            {
                return DataRowToXml((DataRow)source);
            }
        ...
        /// <returns></returns>
        private StringBuilder DataRowToXml(DataRow dataRow)
        {
            StringBuilder _xmlText = new StringBuilder();
            _xmlText.AppendLine("<DataRow>");
            foreach (DataColumn col in dataRow.Table.Columns)
            {
                _xmlText.AppendFormat("<{0}>{1}</{0}>",col.ColumnName,dataRow[col]);
            }
            _xmlText.Append("</DataRow>");
            return _xmlText;
        }
